    ...
    <div className="border border-line bg-white p-5">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <BookText size={18} className="text-loop" />
          <h2 className="text-xl font-black text-ink">FAQ Widget Setup</h2>
        </div>
        <span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-bold text-slate-700">Docs: {uploadedCount}</span>
      </div>
      ...
      <div className="mt-4 grid gap-3">
        <input
          ...
          className="border border-line px-3 py-2 text-sm outline-none focus:border-loop"
        />
        <input
          value={title}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          onClick={addDoc}
          disabled={pending}
          className="inline-flex items-center gap-2 rounded-md border border-line bg-white px-3 py-2 text-sm font-bold text-slate-700 disabled:opacity-60"
        >
          <Plus size={15} />
          Add doc only
        </button>
        <button
          ...
          disabled={pending}
          className="inline-flex items-center gap-2 rounded-md bg-loop px-3 py-2 text-sm font-bold text-white disabled:opacity-60"
        >
      ...
      {snippet ? (
        <textarea readOnly value={snippet} rows={3} className="mt-3 w-full border border-line bg-slate-50 px-3 py-2 text-xs text-slate-700" />
      ) : null}
